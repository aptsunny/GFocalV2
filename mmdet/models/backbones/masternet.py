...
class MasterNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.classfication = task == 'classification'
    ...
    def get_model_size(self, return_list=False):
        ...
        if self.classfication:
            model_size += self.block_list[
                -1].out_channels * self.num_classes + self.num_classes  # for fc_linear
            model_size_list.append(self.block_list[-1].out_channels
    ...
    def get_flops(self, resolution):
        ...
        if self.classfication:
            flops += self.block_list[
                -1].out_channels * self.num_classes  # for fc_linear

        return flops

    def get_latency(self, benchmark_func):
        return benchmark_func(self)
    ...
    def entropy_forward_pre_GAP(self,
                                x,
                                skip_relu=True,
                                skip_bn=True,
                                **kwarg):
